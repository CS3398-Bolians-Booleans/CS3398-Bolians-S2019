...
class DecisionTree:
    ...
    def predict(self, X):
        ...
        result = result.to_numpy(dtype=np.int8)
        result = self.action_ohe.inverse_transform(result)
        result = self.action_le.inverse_transform(result.astype(dtype=np.int8))
